...
def main(cfg: DictConfig) -> None:
    ...
    lerobot_dataset = LeRobotDataset(**dataset_kwargs)
    print(f"Dataset loaded: {lerobot_dataset.num_episodes} episodes, {lerobot_dataset.num_frames} frames")
    ...
    if image_key not in lerobot_dataset.features:
        ...
        camera_keys = lerobot_dataset.meta.camera_keys if hasattr(lerobot_dataset.meta, 'camera_keys') else []
        if camera_keys:
            image_key = camera_keys[0]
        else:
            raise ValueError(f"Could not find image key in dataset. Available keys: {list(lerobot_dataset.features.keys())}")
    ...
    print(f"Processing {num_episodes} episodes (first frame of each)...")
    ...
    # Process each episode (first frame only)
    for episode_idx in range(num_episodes):
        ...
        try:
            ...
            if hasattr(lerobot_dataset, 'episode_data_index'):
                first_frame_idx = lerobot_dataset.episode_data_index["from"][episode_idx].item()
            elif hasattr(lerobot_dataset.meta, 'episodes'):
                first_frame_idx = lerobot_dataset.meta.episodes["dataset_from_index"][episode_idx]
            else:
                # Fallback: assume episodes are sequential
                frames_per_episode = lerobot_dataset.num_frames // lerobot_dataset.num_episodes
                first_frame_idx = episode_idx * frames_per_episode
            
            # Get first frame data
            frame_data = lerobot_dataset[first_frame_idx]
            
            # Extract image
            img_tensor = frame_data[image_key]
            
            # Get episode and frame indices
            ep_idx = int(frame_data["episode_index"].item()) if "episode_index" in frame_data else episode_idx
            frame_idx_in_ep = int(frame_data["frame_index"].item()) if "frame_index" in frame_data else 0
            ...
            if not instruction:
                ...
                if task_idx is not None:
                    ...
                    if hasattr(lerobot_dataset.meta, 'tasks'):
            ...
            if not instruction:
                instruction = f"Episode {ep_idx}"
            
            # Visualize and save
            output_path = output_dir / f"episode_{ep_idx}_frame_{frame_idx_in_ep}.png"
            visualize_bbox(image_pil, bboxes, str(output_path), ep_idx, frame_idx_in_ep, instruction=instruction)
...
def prefill(
    *,
    ...
    content: List[Dict] = [{"type": "image", "image": image}, {"type": "text", "text": task}, {"type": "text", "text": prompt_sep_text}, {"type": "text", "text": prompt}]
...
def generate_bbox(
    ...
    hidden_states, kv_cache, cache_position, attention_mask, next_token = prefill(
        image=image, task=task, prompt=prompt, model=model, processor=processor, device=device
    )
    decoded = decode(
        model=model,
        processor=processor,
...
def visualize_bbox(
    ...
    if instruction:
        ...
        for line in lines:
            ...
            img_draw = Image.alpha_composite(img_draw.convert('RGBA'), bg_img).convert('RGB')
            draw = ImageDraw.Draw(img_draw)
    ...
    info_bg_coords = [img_draw.width - len(info_text) * 8 - 30, info_y - 5, img_draw.width - 10, info_y + info_height + 5]
    draw.rectangle(info_bg_coords, fill=(128, 128, 128))
    draw.text((img_draw.width - len(info_text) * 8 - 25, info_y), info_text, fill=(255, 255, 255), font=font)
